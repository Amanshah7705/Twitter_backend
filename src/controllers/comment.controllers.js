    ...
    try {
         ...
         if(underwhichtweet){
                 if(underwhichtweet && underwhichcomment){
                    ...
                    const checkcomment = await CommentModel.find({_id:underwhichcomment})
                    if(!checkcomment){
                        ...
                        res.status(200).json(new APIRESPONCE(200,"Your Comment Created",newcomment))
                 }
         ...
         throw new APIERROR(501,"error occur at comment add movement",error.errors)
    }
})
...
const ShowCommentOfTweet = HandleMiddleware(async(req,res,next)=>{
     ...
     const AllTweet = await CommentModel.find({ undertweet: tweetid }).sort({ createdAt: 1 }).select({createdAt:0,updatedAt:0,__v:0});
     let acc = {};
