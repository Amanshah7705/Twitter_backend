...
const ProfileDeatils = HandleMiddleware(async (req, res, next) => {
  try {
    ...
    let meka = await totaldata[0].TotalFollowingPart[0].followers.find(item => item.toString() === mydata._id.toString())
    if (meka) {
      meka = true
    }
    else {
...
export { RegisterUser, LoginUser, GenerateAccessToken, GenerateRefreshToken, ForgotPassword, NewPassword, UpdateDetails, FollowList, UnFollowList, UserDeatils, TimePass, ProfileDeatils, SearchBar }